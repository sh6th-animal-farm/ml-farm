...
function addImageBox(src, savedFileName) {
    // 미리보기 박스 생성
    const box = AdminRenderer.createImageBox(
        src,
        savedFileName,
        // 제거 버튼 눌렀을때 동작
        function (e) {
...
async function selectProject(data) {
    ...
    document.getElementsByName("project_description")[0].value =
        data.projectDescription || "";
    document.getElementsByName("target_amount")[0].value = data.targetAmount;
    document.getElementsByName("min_amount_per_investor")[0].value =
        data.minAmountPerInvestor;
    document.getElementsByName("expected_return")[0].value =
        data.expectedReturn;
    ...
    try {
        ...
        if (picturesData) {
            AdminRenderer.displayExistingImages(
                picturesData,
                previewList,
                addBtn,
                markImageAsDeleted,
    ...
    try {
        ...
        if (tokenData) {
            ...
            document.getElementsByName("token_name")[0].value =
                tokenData.tokenName || "";
            document.getElementsByName("ticker_symbol")[0].value =
                tokenData.tickerSymbol || "";
            document.getElementsByName("total_supply")[0].value =
...
function getFormData() {
    ...
    return FormUtil.adminProjectFormToJSON(
        formData,
        selectedProjectId,
        deletedPictureIds,
    );
...
function updateProject() {
    ...
    ProjectApi.update(data).then((text) => {
        console.log(text);
        if (text === "success") {
            PendingManager.setPending("수정되었습니다.");
            window.location.href = `${ctx}/admin/project/new`;
        } else {
            const errorMsg = text;
            ToastManager.show("실패: " + errorMsg);
        }
    });
}
