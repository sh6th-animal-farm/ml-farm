...
const transformSingleVolume = (data) => ({
    ...
    color:
        Number(data.closingPrice) >= Number(data.openingPrice)
            ? "rgba(242, 54, 69, 0.3)"
            : "rgba(8, 153, 129, 0.3)",
});
...
document.querySelectorAll(".tab-menu").forEach((tabMenu) => {
    tabMenu.addEventListener("click", (e) => {
        ...
        card.querySelectorAll(".tab-content-wrapper").forEach((tab) =>
...
function updateOrderTotal(tabContent) {
    ...
    if (isBuyTab) {
        if (orderType === "MARKET") {
            ...
            // 지정가 매수: 가격 * 수량
            const total =
                getValue(".price-group .input-box") *
                getValue(".volume-group .input-box");
...
document
    ...
    .addEventListener("click", (e) => handleOrder(e, "BUY"));
document
    .querySelector(".btn-sell")
    .addEventListener("click", (e) => handleOrder(e, "SELL"));
...
async function handleOrder(event, side) {
    ...
    const getValue = (selector) => {
        ...
        return element && element.value
            ? Number(element.value.replace(/,/g, ""))
            : 0;
    };
        ...
        totalPrice:
            side === "BUY"
                ? orderType === "LIMIT"
                    ? price * volume
                    : amount
                : 0, // 지정가 매수는 가격*수량, 시장가 매수는 총액, 매도는 0
    };
...
function renderPendingOrders(pendingList) {
    ...
    if (!pendingList || pendingList.length === 0) {
        listContainer.innerHTML =
            '<li class="transaction-item no-content">미체결 내역이 없습니다.</li>';
        return;
    }
...
document.querySelectorAll(".tab-menu").forEach((tabMenu) => {
    tabMenu.addEventListener("click", (e) => {
        ...
        // 버튼 active
        tabMenu
            .querySelectorAll(".trade-btn")
            .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");

        // 탭 내용 전환
        const card = btn.closest(".trade-card");
        card.querySelectorAll(".scroll").forEach((tab) =>
            tab.classList.remove("active"),
        );
        card.querySelector(`#${tabId}`).classList.add("active");
    });
...
function renderTradeHistory(tradeList) {
    ...
    if (!tradeList || tradeList.length === 0) {
        tbody.innerHTML =
            '<tr><td colspan="3" class="no-content">체결 내역이 없습니다.</td></tr>';
        return;
    }
...
function renderOrderBook(sellList, buyList) {
    ...
    if (sellList.length === 0 && buyList.length === 0) {
        tbody.innerHTML =
            '<tr><td colspan="3" class="no-content">호가 정보가 없습니다.</td></tr>';
        return;
    }

    // 매도: 가격 낮은 순 15개 (오름차순)
    const sortedSells = [...sellList]
        .sort((a, b) => a.price - b.price)
        .slice(0, 15);
    // 매수: 가격 높은 순 15개 (내림차순)
    const sortedBuys = [...buyList]
        .sort((a, b) => b.price - a.price)
        .slice(0, 15);

    // 매도 내림차순(reverse) -> 매수 내림차순
    const sellHtml = sortedSells
        .reverse()
        .map((item) => createOrderRowHtml(item, "SELL"))
        .join("");
    const buyHtml = sortedBuys
        .map((item) => createOrderRowHtml(item, "BUY"))
        .join("");
...
function updateOrderBook(data) {
    ...
    const targetList =
        data.side === "BUY" ? window.orderBuyList : window.orderSellList;
